<!doctype html>
<html lang="fr">
    <head>
        <meta charset="UTF-8">
        <!--[if IE]><meta http-equiv="X-UA-Compatible" content="IE=edge"><![endif]-->
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>RB web developpement - porfolio</title>
        <link rel="stylesheet" href="./public/css/style.css?v=<?= time() ?>" media="all">
    </head>
    <body class="univers-leroy-merlin" id="/home">
        <header>
            <h1><?= $_SERVER['PHP_SELF'] ?></h1>
        </header>
        <div class="global">
            <nav>
                <ul class="main">
            	    <li><a href="#/home">&#8962;</a></li>
            	    <li><a href="#" id="iconPrev">&#59237;</a></li>
            	    <li><a href="#/competences">&#9871;</a></li>
            	    <li class="home-no-info"><a href="#/competences" id="iconInfo">&#59141;</a></li>
            	    <li><a href="#" id="iconNext">&#59238;</a></li>
                    <li class=""><a href="#about" id="iconHelp">&#59140;</a></li>
            	    <li><a href="#contact">&#9993;</a></li>
            	    <li><a href="#contact">&#62233;</a></li>
            	</ul>
            </nav>
            <section id="/competences"></section>
        </div>
        <script defer src="./public/js/app.js?v=<?= time() ?>"></script>
    </body>
</html>
